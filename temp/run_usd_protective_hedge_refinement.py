...
def _scenarios() -> list[dict[str, object]]:
    ...
    for green3_floor in (0.035, 0.040, 0.045, 0.050):
        for green2_floor in (0.000, 0.025, 0.050):
            for red_floor in (0.000, 0.100, 0.200):
                floors = {
                    4: 0.00,
                    3: green3_floor,
                    2: green2_floor,
                ...
                scenarios.append(
                    {
                        "name": (
                            f"refine_g3_{green3_floor:.3f}"
                            f"_g2_{green2_floor:.3f}"
                            f"_r_{red_floor:.3f}"
                        ),
...
def _write_report(
    ...
    lines.extend(
        [
            ...
            (
                f"- Highest final-value candidate: `{highest_usd['name']}` "
                f"with ${float(highest_usd['final_portfolio_value_usd']):,.2f} "
                f"and {float(highest_usd['max_drawdown_pct']):.3f}% max DD."
            ),
            (
                "- No refined hedge variant improved max DD while also preserving final USD "
                "at or above the signal-governor anchor."
...
def main() -> None:
    ...
    out_dir = Path("reports") / f"usd_protective_hedge_refinement_{datetime.now():%Y%m%d_%H%M%S}"
